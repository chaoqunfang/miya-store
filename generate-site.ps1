...
foreach ($cat in $categories) {
    Write-Host "  $($cat.Name): $($cat.Count) products"
}
  ...
  <header class="header">
    <div class="header-content">
      ...
      <nav class="nav">
        ...
        <a href="#bags">Bags</a>
        <a href="https://wa.me/8615759948239" target="_blank">Contact</a>
      </nav>
    </div>
  </header>
...
$catIndex = 0
foreach ($cat in $categories) {
    $catId = "category-$catIndex"
    $html += "  <div class=`"category-section`" id=`"$catId`">`n"
    $html += "    <h2 class=`"category-title`">$($cat.Name) ($($cat.Count))</h2>`n"
    $html += "    <div class=`"products-grid`">`n"
    
    $prodIndex = 0
    foreach ($p in $cat.Group) {
        $imgFile = "images/product-{0:D3}.jpg" -f ($products.IndexOf($p) + 1)
        $title = $p.title -replace "'", "&#39;" -replace '"', '&quot;'
        $waMsg = [System.Web.HttpUtility]::UrlEncode("Hi, I'm interested in: $($p.title)")
        
        $html += "      <div class=`"product-card`">`n"
        $html += "        <img src=`"$imgFile`" alt=`"$title`" class=`"product-image`">`n"
        $html += "        <div class=`"product-info`">`n"
        $html += "          <div class=`"product-title`">$title</div>`n"
        $html += "          <a href=`"https://wa.me/8615759948239?text=$waMsg`" target=`"_blank`" class=`"inquire-btn`">Inquire Now</a>`n"
        $html += "        </div>`n"
        $html += "      </div>`n"
...
$html += @'
  <footer class="footer">
    <div class="footer-content">
      ...
      <div class="footer-section">
        <h3>Contact</h3>
        <a href="https://wa.me/8615759948239" target="_blank">WhatsApp: +86 157 5994 8239</a>
      </div>
      <div class="footer-section">
        <h3>Follow Us</h3>
        <a href="#">Instagram</a>
